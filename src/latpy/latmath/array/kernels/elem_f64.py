"""
latmath.array.kernels.elem_f64

Elementwise kernels for f64.
"""
...
__all__ = ["add_f64", "mul_f64", "add_scalar_f64", "mul_scalar_f64", "cmp_f64", "cmp_scalar_f64", "where_f64"]
...
def add_scalar_f64(A, s: float, OUT):
...
def mul_scalar_f64(A, s: float, OUT):
    if A.shape != OUT.shape:
...
def cmp_scalar_f64(A, val: float, OUT, op: str):
    if A.shape != OUT.shape:
        raise ShapeError("cmp: shape mismatch")
    n = numel(A.shape)
    if n == 0: return OUT
...
def _cmp_scalar_strided(A, val: float, OUT, op: str):
    shape, ndim = A.shape, A.ndim
    a_buf, o_buf = A._buf, OUT._buf
    ao, oo = A.offset, OUT.offset
    as_, os_ = A.strides, OUT.strides
